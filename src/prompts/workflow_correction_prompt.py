...
2. Task nodes must use ids starting at "-1001" and decrementing (-1001, -1002, -1003, ...).
3. End nodes must use ids lower than any task id (e.g., -2000, -2001).
...
Every variable used anywhere in the workflow MUST be declared in Start "variables" array.
...
For Compare nodes with true/false branches: use ONE link object with both branches.
...
Pattern 1 — Add conditional check to existing chain:
  OLD:  Start(-1000) → Task(-1001) → End(-2000)
  NEW:  Start(-1000) → Task(-1001) → Compare(-1002) → [true: End(-2000) | false: End(-2001)]
  How:  Insert new Compare node, update existing Links, add new Link for false path

Pattern 2 — Add sequential task:
  OLD:  Start(-1000) → Task(-1001) → End(-2000)
  NEW:  Start(-1000) → Task(-1001) → NewTask(-1002) → End(-2000)
  How:  Insert new task node, update Links: old Task links to new Task, new Task links to End

Pattern 3 — Add retry logic:
  OLD:  Start(-1000) → Task(-1001) → End(-2000)
  NEW:  Start(-1000) → Task(-1001) → Compare(-1002) → [true: End(-2000) | false: Retry(-1003)]
               ↑ ← ← ← ← ← ← ← ← ← ← ← ← ← ← ← ← Increment(-1004) → Compare loop
  How:  Insert Compare node with true/false, add Increment loop back to task

Pattern 4 — Replace failing task with working alternative:
  OLD:  Start(-1000) → OldTask(-1001) → End(-2000)
  NEW:  Start(-1000) → NewTask(-1001) → End(-2000)
  How:  Replace node content, keep id and links unchanged

--- OUTPUT FORMAT (CRITICAL - READ THIS) ---

⚠️ You MUST return a JSON object with EXACTLY two top-level keys:
  1. "workflow" key → contains FULL corrected workflow with Start, all TaskNodes, Links
  2. "explanation" key → contains your explanation text

DO NOT return:
  ❌ Just the workflow without wrapper: {{"Start": [...], "CmdStage": [...], "Links": [...]}}
  ❌ Nested workflow: {{"workflow": {{"workflow": {...}}}}}
  ❌ Missing explanation: {{"workflow": {{...}}}}  (must include "explanation" key)
  ❌ Explanation outside "explanation" field

MUST RETURN:
  ✅ {{"workflow": {{"Start": [...], "CmdStage": [...], "Links": [...]}}, "explanation": "..."}}

--- Explanation Format (MANDATORY STRUCTURE) ---
Must contain these 4 sections (use line breaks between sections):

PROBLÈMES:
[List each broken issue in the original workflow. Example: "Variable $X not declared in Start", "Link references invalid id -1010"]

FIXES:
[List specific changes you made. Example: "Added $X to Start variables", "Updated link -1001 → -1002 to -1001 → -1002"]

POURQUOI:
[Explain why the fixes work. Example: "By declaring $X in Start, RULE A is satisfied. Tasks can now use $X"]

RÉSULTAT:
[Describe the corrected workflow sequence. Example: "Flow is now: Start → CmdStage → Compare → [true: End | false: Sleep → CmdStage]"]

--- Pre-submission validation checklist ---
Before returning your JSON, verify:
☐ Output structure is {{"workflow": {{...}}, "explanation": "..."}}
☐ "workflow" contains Start, task nodes (CmdStage/AppStage/etc.), Links
☐ All variables used in tasks are declared in Start.variables
☐ No duplicate output variables (each task output is unique)
☐ All Links reference valid existing ids
☐ Start id is exactly -1000
☐ Task ids decrement: -1001, -1002, -1003, ...
☐ End ids are < -2000
☐ Enum parameters (compare_type, arithmetic_ops, etc.) are NOT $ variables
☐ Explanation has PROBLÈMES, FIXES, POURQUOI, RÉSULTAT sections
☐ NO markdown, NO comments, NO extra nesting
☐ Output is VALID JSON (no trailing commas, all quotes closed)
"""

CANONICAL_EXAMPLE_CORRECTION = """
--- Canonical example of CORRECTED workflow ---

SCENARIO: User says "add a check to verify the ping result"
ORIGINAL had: Start → CmdStage → End
CORRECTED has: Start → CmdStage → Compare → [true: End | false: End]

{
  ...
  "explanation": "PROBLÈMES: No result verification; ping always succeeds even if connection fails.\n\nFIXES: Added CompareStrings node (-1002) to check if ping output contains '0 packets lost'. Updated Links to route true/false to separate End nodes.\n\nPOURQUOI: CompareStrings evaluates $PING_RESULT, creating a branch point. This satisfies RULE A (all variables in Start), RULE C (each link once), and Pattern 1 (add conditional).\n\nRÉSULTAT: Workflow now executes: Start → CmdStage → CompareStrings → if success then End(-2000), else End(-2001). On device, failed pings are logged separately."
}
"""
