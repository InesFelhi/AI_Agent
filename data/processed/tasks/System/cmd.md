# Cmd Stage

## Summary

- **Internal name**: `CmdStage`
- **Category**: System
- **Purpose**: Execute a shell command on the Android device, with optional root privilege handling.
- **Task type**: Normal

## Compatibility

- **Minimum AndroMate version**: `{{ ANDROMATE_FIRST_VERSION }}`
- **Maximum AndroMate version**: `{{ ANDROMATE_CURRENT_VERSION }}`
- **Minimum Android**: `{{ ANDROMATE_MIN_APP_SDK }}`
- **Maximum Android tested**: `{{ ANDROID_CURRENT_APP_SDK }}`
- **Supported manufacturers**:
  - ✅ Samsung (One UI 6.x / 7.x / 8.x)
- **Required permissions**:
  - `INTERNET` (if the command performs network access)
  - `MANAGE_EXTERNAL_STORAGE` (if the command reads certain files on the device)
  - ⚠️ **Root required** for certain commands (depending on the value of `root`)

## Detailed description

The **Cmd Stage** task allows executing a *shell* command directly on the Android device.  
It is used to:

- Test network connectivity (`ping`, `curl`, `traceroute`, etc.)
- Collect system information (CPU, memory, network interfaces, radio…)
- Modify configuration (if root: `ip`, `ifconfig`, `tc`, etc.)
- Automate technical diagnostics or repetitive system actions.

The task handles:

- execution with or without **root privileges**,
- retrieval of **standard output (stdout)**,
- retrieval of **error output (stderr)**.

## Input parameters

- Parameter: `cmd_text` | Type: String | Required: Yes | Possible values: Any valid shell command | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: —
- Parameter: `root` | Type: Boolean | Required: No | Possible values: true / false | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: false

## Output parameters

- Field: `cmd_result_output` | Type: String | Trigger condition: When the main command **succeeds** | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: `<ANDROMATE_NULL_VALUE>`
- Field: `cmd_error_output` | Type: String | Trigger condition: When the main command **fails** | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: `<ANDROMATE_NULL_VALUE>`

## Execution flowchart

Diagram Nodes:
- ResolveParams: 🔄 Resolve Parameters cmd_text, root
- ExecRoot: ⚙️ Execute with root su -c command
- ExecCmd: ⚙️ Execute Command sh -c command
- ReadOutput: 📤 Read stdout and stderr Max 10000 characters
- StoreError: 💾 Store in cmd_error_output Error message
- StoreSuccess: 💾 Store in cmd_result_output Command output
- LogReport: 📋 Log Report ReportSection

Workflow Flow:
- 🔄 Resolve Parameters cmd_text, root → CheckRoot
- ⚙️ Execute with root su -c command → 📤 Read stdout and stderr Max 10000 characters
- ⚙️ Execute Command sh -c command → 📤 Read stdout and stderr Max 10000 characters
- 📤 Read stdout and stderr Max 10000 characters → CheckError
- 💾 Store in cmd_error_output Error message → 📋 Log Report ReportSection
- 💾 Store in cmd_result_output Command output → 📋 Log Report ReportSection
- 📋 Log Report ReportSection → Success

**How it works:**

1. **Resolve parameters**: Loads cmd_text and root from configuration
2. **Check root**: Determines if command should be executed with root privileges
3. **Execute command**: Runs via `/system/bin/sh -c` (or `su -c` if root=true)
4. **Read output**: Captures stdout and stderr, limited to 10,000 characters max
5. **Check for errors**: If stderr is not empty, captures error message
6. **Store result**: Stores in `cmd_result_output` (success) or `cmd_error_output` (error)
7. **Log**: Records execution report
8. **Result**: Returns CmdTaskResult (always succeeds, errors are in variables)

**Note**: This task does NOT throw exceptions. Errors are captured in `cmd_error_output` variable.

## Possible Error Messages

The task captures error messages from the command execution in `cmd_error_output`. These messages come from:

1. **Command execution errors** (stderr from the process)
2. **System-level errors** (process timeout, device not rooted, etc.)

## Error Message Categories

### Network Errors

- Error Message: `Network is unreachable` | Cause: No network connectivity | Example Command: `ping 8.8.8.8`
- Error Message: `Temporary failure in name resolution` | Cause: DNS resolution failure | Example Command: `ping google.com`
- Error Message: `Connection refused` | Cause: Target port closed | Example Command: `curl http://localhost:9999`
- Error Message: `Connection timeout` | Cause: Server not responding | Example Command: `curl http://192.0.2.1`
- Error Message: `Operation timed out` | Cause: Network latency too high | Example Command: `ping 8.8.8.8 -c 10`

### File System Errors

- Error Message: `No such file or directory` | Cause: File doesn't exist | Example Command: `cat /data/nonexistent.txt`
- Error Message: `Permission denied` | Cause: Insufficient permissions | Example Command: `cat /root/private.txt` (without root)
- Error Message: `Is a directory` | Cause: Trying to read directory as file | Example Command: `cat /data/`
- Error Message: `Device or resource busy` | Cause: File/device locked | Example Command: `rm /data/locked_file`

### Root/Privilege Errors

- Error Message: `Permission Denied (no rooted Device)` | Cause: root=true but device not rooted | Example Command: Any command with `root: true`
- Error Message: `su: not found` | Cause: su binary not available | Example Command: Root command on non-rooted device
- Error Message: `Operation not permitted` | Cause: Action requires root | Example Command: `ip route add` without root

### Command Execution Errors

- Error Message: `command not found` | Cause: Binary/executable doesn't exist | Example Command: `iperf3` (not installed)
- Error Message: `Process timed out and was killed` | Cause: Timeout exceeded (300s) | Example Command: Long-running command
- Error Message: `Exec error: ...` | Cause: IOException during execution | Example Command: Corrupted command or system issue

### System Information Errors

- Error Message: `cannot read ...` | Cause: Cannot access system file | Example Command: `cat /proc/cpuinfo` (permission denied)
- Error Message: `unknown option` | Cause: Invalid command argument | Example Command: `ping --invalid-option`
- Error Message: `Bad syntax` | Cause: Malformed command | Example Command: `sh -c "echo unclosed`

## Error Message Structure

Error messages in `cmd_error_output` follow this structure:

```
[Timestamp] [Error Type]: [Error Description]
Line 1 of stderr output
Line 2 of stderr output
... (up to 10,000 characters total)
```

## Handling Errors in Workflows

### Example 1: Check for network error

```json
{
  "CmdStage": [
    {
      "id": "1",
      "title": "Test Network",
      "cmd_text": "ping -c 1 8.8.8.8",
      "root": false,
      "cmd_result_output": "$PING_RESULT",
      "cmd_error_output": "$PING_ERROR"
    }
  ],
  "Condition": [
    {
      "id": "2",
      "condition": "$PING_ERROR contains 'unreachable'",
      "then": "NetworkDownTask",
      "else": "ContinueTask"
    }
  ]
}
```

### Example 2: Retry with root if permission denied

```json
{
  "CmdStage": [
    {
      "id": "1",
      "title": "First try without root",
      "cmd_text": "cat /system/build.prop",
      "root": false,
      "cmd_result_output": "$BUILD_PROP",
      "cmd_error_output": "$ERROR_1"
    }
  ],
  "Condition": [
    {
      "id": "2",
      "condition": "$ERROR_1 contains 'Permission denied'",
      "then": "RetryWithRootTask",
      "else": "SuccessTask"
    }
  ]
}
```

## Shell Detection

The task automatically detects if a shell is needed based on conditions:

- Command **contains spaces** (ex: `ping -c 1 8.8.8.8`)
- Command **contains special characters**: `;` `|` `&` `<` `>` `*` `(` `)` `{` `}` `[` `]`

If one of these conditions is true, the command is executed via `/system/bin/sh -c "command"`.

## Examples

- Command: `ping` | Needs shell?: ❌ No | Reason: No spaces, no special chars
- Command: `ping -c 1 8.8.8.8` | Needs shell?: ✅ Yes | Reason: Contains spaces
- Command: `ping 8.8.8.8 | grep` | Needs shell?: ✅ Yes | Reason: Contains pipe `|`
- Command: `curl http://example.com && echo ok` | Needs shell?: ✅ Yes | Reason: Contains `&&`
- Command: `cat /data/file > /tmp/out` | Needs shell?: ✅ Yes | Reason: Contains `>` (redirection)

## Limits

- Limit: **Timeout** | Value: 300 seconds (5 min) | Description: Maximum time before process termination
- Limit: **Output size** | Value: 10,000 characters | Description: Maximum for stdout + stderr combined
- Limit: **Escaped characters** | Value: `"` → `\"` | Description: Quotes are escaped in root commands

### Exceeding Limits

- **Timeout exceeded**: Process is killed and `CMD_PROCESS_TIMEOUT` error is raised
- **Output too large**: Text is truncated with `... output truncated ...` added at the end

## Input parameter details

### 1. Input parameter: `cmd_text`

The main command to execute on the device.

#### Example

```json
"cmd_text": "ping -c 1 www.google.com"
```

#### Details

- Executed via the Android shell (`/system/bin/sh`).
- Must be compatible with the Android version and the binaries available on the device.
- Its standard output is passed to `cmd_result_output` using an AndroMate variable (e.g., `$RESULT`).

### 2. Input parameter: `root`

Indicates whether the command should be executed with **root** privileges.

#### Example

```json
"root": true
```

#### Details

- If `root = true`, the command is usually executed via `su -c "<cmd_text>"`.
- If the device is not rooted or `su` is unavailable, the task may fail.
- Use only for actions requiring elevated permissions (low-level networking, system files, etc.).

## Output parameter details

### 3. Result variable: `cmd_error_output`

`cmd_error_output` is an **internal variable** used to **store the error output (stderr)** produced by the main command (`cmd_text`) when it fails.

### Example

```json
"cmd_error_output: $CMD_ERROR"
```

### Trigger

`cmd_error_output` is filled when the main command:

- returns an **exit code ≠ 0**,
- exceeds a **timeout**,
- or when the command is **not found or not executable**.

In all these cases, the error output (stderr) is copied into the variable.

### Use cases

- Save the error generated by a command (`ping`, `iperf`, `twamp`, `curl`…)
- Record specific logs (`logcat`, internal traces…)
- Send the error to a backend for analysis or monitoring
- Use the error in another task (condition, fallback, report…)

### 4. Result variable: `cmd_result_output`

`cmd_result_output` is an **internal variable** used to **store the standard output (stdout)** produced by the command executed in the task.

### Example

```json
"cmd_result_output: $RESULT"
```

### Details

- `$RESULT` contains **the standard output of the main command**.
- The workflow can:
  - analyze the output,
  - extract a value (response time, code, state…),
  - transform or normalize the result,
  - send it to a backend (via `curl` or `Http request`).

### Use cases

- Send metrics to a collection server
- Chain the execution of multiple dependent commands

## Complete JSON example

```json
{
  "CmdStage": [
    {
      "id": "-1",
      "title": "Cmd Stage",
      "cmd_text": "ping -c 1 www.google.com",
      "root": true,
      "cmd_error_output": "$PING_GOOGLE_CMD_ERROR",
      "cmd_result_output": "$PING_GOOGLE_CMD_RESULT"
    }
  ]
}
```