...
- Field: `offset_ms_output` | Type: Long | Trigger condition: When NTP sync succeeds | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: `<ANDROMATE_NULL_VALUE>`
- Field: `rtt_ms_output` | Type: Long | Trigger condition: When NTP sync succeeds | Android Compatibility: {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | AndroMate Compatibility: {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | Default: `<ANDROMATE_NULL_VALUE>`

## Exceptions

- Code: `SYNC-NTP-TASK-001` | Exception Name: NTP Client Error | Description: Failed to communicate with NTP server (IOException) or received invalid response.
