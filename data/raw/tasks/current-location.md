...
|-----------|------|----------|-----------------|----------------------|-------------------------|---------|
| `location_timeout` | Integer | Yes | Time in milliseconds | {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | 10000 |
...
|-------|------|------------------|----------------------|-------------------------|---------|
| `location_latitude_output` | Double | When location is successfully retrieved | {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | `<ANDROMATE_NULL_VALUE>` |
| `location_longitude_output` | Double | When location is successfully retrieved | {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | `<ANDROMATE_NULL_VALUE>` |
| `location_accuracy_output` | Float | When location is successfully retrieved | {{ ANDROMATE_MIN_APP_SDK }} → {{ ANDROID_CURRENT_APP_SDK }} | {{ ANDROMATE_FIRST_VERSION }} → {{ ANDROMATE_CURRENT_VERSION }} | `<ANDROMATE_NULL_VALUE>` |
...
|------|---------------|-------------|
| GPS_GET_CURRENT_LOCATION_NULL_VALUE | Null Location | Location could not be obtained (null) - the GPS provider returned a null position. |
| GPS_GET_CURRENT_LOCATION_TIMEOUT | GPS Timeout | Failed to obtain GPS position within the specified timeout (location_timeout). |
| GPS_APP_HAS_NO_LOCATION_PERMISSION | Missing Location Permission | The application does not have ACCESS_FINE_LOCATION or ACCESS_COARSE_LOCATION permission. |
